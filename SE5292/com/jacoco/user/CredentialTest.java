...
public class CredentialTest {
	...
	public boolean validateUser(String username, String password) {
		...
		/**
		 * TO-DO Case to validate if user exists in database. Validate if userId
		 * and password match the one in database
		 * 
		 * @return boolean
		 */
	...
	public boolean signNewUser() {
		...
		if (null == user.getName() || null == user.getId() || null == user.getPassword() || null == user.getRole())
			return false;
		if (user.getName().trim().length() == 0 || user.getId().trim().length() == 0
				|| user.getPassword().trim().length() == 0 || user.getRole().equals(Role.Admin))
			return false;

		// validate password format to match alphanumeric
		// create a regex pattern for matching alphanumeric string of number
		// (should have atleast 1 number),
		// special char (atleast 1 special character) and alphabets(at least 1
		// capital) and length >= 8
		final String regex = "(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{8,}";
		if (Pattern.matches(regex, user.getPassword())) {
			// add user to database and return success or failure of insert into
			// db operation
			int insert = 0; // insert will return 0 or 1 based on success
							// failure. assume it's true for now
	...
	public boolean loginUser() {
		...
		// validate if user-id and password match the record from database and
		// return boolean as true or false.
		// based on the return int value. For now, assume it is always validated
		// by the db and return true
	...
	public boolean checkIfUserIDExists() {
		...
		ArrayList<String> UserIDList = new ArrayList<String>(); // fetch all
																// existing
																// UserIDList
																// from DB
